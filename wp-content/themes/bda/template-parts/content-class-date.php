<?php
    ...
    if( have_rows('class_dates') ):

        $upcomming_dates = array();

		$count = 0;

        while ( have_rows('class_dates') ) : the_row();

            if( isset( $fpd ) && $fpd == false ) { // do not filter past date for single event page. fpd = filter past date

                $upcomming_dates[$count]['date'] = get_sub_field( 'date' );
                $upcomming_dates[$count]['start_time'] = get_sub_field('start_time');
                $upcomming_dates[$count]['end_time'] = get_sub_field('end_time');

            } else {

                if( strtotime( get_sub_field('date') )  >= strtotime( date('Ymd') ) ) { // filter the past date for event listing page 
        ...
        foreach( $upcomming_dates as $date ) {
        	...
        	if( isset( $date_format ) == 3 ) { // 3 is just a number to specify the straight line date as in single event page
                ...
                $str .= '<span class="date">' . date( "F j — ", strtotime( $date['date'] ) ) . '<span>' . $date['start_time'];
	            if( $date['end_time'] ) {
	                $str .= ' - ' . $date['end_time'];
	            }
                ...
                $str .= '<span class="date ' . $date_count . '">' . date( 'F j, Y', strtotime( $date['date'] ) ) . '<br>' . $date['start_time'];
        		if( $date['end_time'] ) {
        			$str .= ' - ' . $date['end_time'];
        		}
        		$str .= '</span>';
        ...
        echo 'No date added. Please add date in the class.';
